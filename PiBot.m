classdef PiBot < handle
    ...
    methods
        ...
        function connectToLocaliser(obj, ip)
            ...
            obj.TCP_LOCALISER_IMG = tcpip(ip, 43906, 'NetworkRole', 'client');
            obj.TCP_LOCALISER_IMG.InputBufferSize = 4096*20;
        end
        ...
        function img = getImageFromLocaliser(obj)
      
            fopen(obj.TCP_LOCALISER_IMG);
            
            size_ = 500*500*3;
            data = zeros(size_,1);
            n = 0;

            % Keep reading until whole image is received
            while n < size_
                packet = fread(obj.TCP_LOCALISER_IMG, min([size_-n,obj.TCP_LOCALISER_IMG.InputBufferSize]), 'uint8');
                packet_size = numel(packet);
                data(n+1:n+packet_size) = packet;
                n = n + packet_size;
            end
                    
            fclose(obj.TCP_LOCALISER_IMG);
    
            im = reshape(data, [3 500 500]);%fliplr(shape)); % Matlabs reshape has dimensions backwards to python
            im = permute(im, [3,2,1]); % Flip the ordering of dimensions
            im(:,:,:) = im(:,:,[3,2,1]);
            img = im./255;
